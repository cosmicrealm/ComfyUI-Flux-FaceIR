...
@dataclass
class DebugFlux2Params:
    in_channels: int = 128
    context_in_dim: int = 256
    hidden_size: int = 256
    num_heads: int = 8
    depth: int = 2
    depth_single_blocks: int = 4
    axes_dim: list[int] = field(default_factory=lambda: [8, 8, 8, 8])
    theta: int = 2000
    mlp_ratio: float = 2.0
    use_guidance_embed: bool = False
...
class Flux2(nn.Module):
    ...
    def forward(
        ...
        timestep_emb = timestep_embedding(timesteps, 256).to(dtype=self.time_in.in_layer.weight.dtype)
        vec = self.time_in(timestep_emb)
        if self.use_guidance_embed:
            guidance_emb = timestep_embedding(guidance, 256).to(dtype=self.guidance_in.in_layer.weight.dtype)
